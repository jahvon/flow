...
impl<E: CommandExecutor + 'static> VaultCommands<E> {
    ...
    pub async fn create(&self, name: &str) -> CommandResult<String> {
        self.executor
            .execute::<()>(&["vault", "create", name])
            .await
    }

    pub async fn list(&self) -> CommandResult<Vec<String>> {
        self.executor.execute_json(&["vault", "list"]).await
    }
    ...
    pub async fn switch(&self, name: &str) -> CommandResult<String> {
        self.executor
            .execute::<()>(&["vault", "switch", name])
            .await
    }

    pub async fn remove(&self, name: &str) -> CommandResult<String> {
        self.executor
            .execute::<()>(&["vault", "remove", name])
            .await
    }
}
