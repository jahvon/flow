...
var deleteWorkspaceCmd = &cobra.Command{
	...
	Long: "Delete an existing workspace. File contents will remain in the corresponding directory but the workspace will be " +
		"unlinked from tbox's registry. Note: You cannot delete the current workspace.",
	Args: func(cmd *cobra.Command, args []string) error {
		...
		name := args[0]
		if name == currentConfig.CurrentWorkspace {
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := config.RemoveWorkspace(currentConfig, name); err != nil {
